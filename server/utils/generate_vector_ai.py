...
def convert_eps_to_base64(eps_path):
	try:
		...
		if not os.path.exists(eps_path):
			...
			return JSONResponse(content=json.dumps({"error": f"File not found: {eps_path}"}))
...
async def generate_pdf(background_tasks):
	try:
		...
		image_files = [f for f in os.listdir(folder_path) if f.lower().endswith(('png', 'jpg', 'jpeg', 'eps'))]
		image_files.sort()
