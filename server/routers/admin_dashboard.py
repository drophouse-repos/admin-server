...
from utils.generate_vector_ai import generate_vector_image, generate_zip, generate_pdf, clean_old_data, convert_eps_to_base64
	...
	try:
		...
		if result:
			for order in result:
				if 'images' in order:
					for image in order['images']:
						image_data = applyMask_and_removeBackground(order['images'][image]['img_path'], mask_image_path, order['images'][image]['img_id'])
			...
			if not os.path.exists(zip_path):
				return JSONResponse(content=json_util.dumps({"error": f"File not found: {zip_path}"}))
