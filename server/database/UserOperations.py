...
class UserOperations(BaseDatabaseOperation):
    ...
    async def get_student_order(self) -> list:
        try:
            ...
            verfied_orders = []
            for order in orders:
                if 'user_type' in order and 'status' in order and order['user_type'] == 'student' and order['status'] == 'verified':
                    user_data = user_dict.get(order['user_id'], {})
                    order['images'] = {}
                    if "item" in order:
                        for item in order["item"]:
                            ...
                            if 'toggled' in item and item['toggled'] != False:
                                ...
                                item["img_url"] = generate_presigned_url(img_id, "browse-image-v2")
                            order['images'][user_data['first_name']+"_"+user_data['last_name']+"_"+item['size']] = {}
                            order['images'][user_data['first_name']+"_"+user_data['last_name']+"_"+item['size']]["img_path"] = item['img_url']
                            order['images'][user_data['first_name']+"_"+user_data['last_name']+"_"+item['size']]["img_id"] = item['img_id']
