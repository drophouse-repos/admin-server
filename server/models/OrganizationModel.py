...
class Product(BaseModel):
    ...
    mask: Optional[bytes] = Field(None, description="Product mask as Base64 encoded image.")
    # colors: Dict[str, Optional[constr(min_length=1)]] = Field(default_factory=dict, description="Product colors.")
    # clip: Optional[bytes] = Field(b"data:image/png;base64", description="product Clip image as Base64 encoded image.")
    colors: Dict[str, Optional[Color]] = Field(default_factory=dict, description="Product colors.")
    defaultProduct: Optional[bytes] = Field(b"data:image/png;base64", description="Default product image as Base64 encoded image.")
...
class OrganizationModel(BaseModel):
    ...
    mask: Optional[bytes] = Field(None, description="Organization mask as Base64 encoded image.")
    logo: Optional[bytes] = Field(None, description="Organization logo as Base64 encoded image.")
    green_mask: Optional[bytes] = Field(None, description="Organization Green Mask as Base64 encoded image.")
    ...
    favicon: Optional[constr(min_length=1)] = Field(None, description="Organization favicon.")
