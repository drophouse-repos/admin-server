...
class EmailService:
    ...
    def notify_error(self, exception):
        ...
        for exc_head in exception:
            message_body = (
                message_body + f"<strong>{exc_head}:</strong> {exception[exc_head]}<br>"
            )
    ...
    def notify_imagegen_fail(self, idx, task_id, task_info, progress_time):
        ...
        message_body = (
            message_body
            + f"<br><strong>Image Generation took:</strong> {timetaken}<br>\
        ...
        for i in range(len(prompts)):
            ...
            if idx == i:
                style = (
                    "color : blue; border:solid 1px black; border-collapse: collapse;"
                )

            message_body = (
                message_body
                + f"<tr style='{style}'><td style='border:solid 1px black; border-collapse: collapse;'>1</td><td style='border:solid 1px black; border-collapse: collapse;'>{i}</td><td style='border:solid 1px black; border-collapse: collapse;'>{progress_time[i] if i in progress_time else '-- seconds'}</td><td style='border:solid 1px black; border-collapse: collapse;'>{prompts[i]}</td><td style='border:solid 1px black; border-collapse: collapse;'>{'Failed' if isinstance(images[i], Exception) else 'Passed'}</td></tr>"
            )
            message_body = (
                message_body
                + f"<tr style='{style}'><td style='border:solid 1px black; border-collapse: collapse;'>2</td><td style='border:solid 1px black; border-collapse: collapse;'>{i}</td><td style='border:solid 1px black; border-collapse: collapse;'>{progress_time[i + 3] if (i + 3) in progress_time else '-- seconds'}</td><td style='border:solid 1px black; border-collapse: collapse;'>{prompts[i]}</td><td style='border:solid 1px black; border-collapse: collapse;'>{'Failed' if isinstance(images[i+3], Exception) else 'Passed'}</td></tr>"
            )
